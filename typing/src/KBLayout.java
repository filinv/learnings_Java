...
public class KBLayout {
    ...
    if (s_char.charAt(0) == '\\'){
    try{
    c = (char)(Integer.parseInt(s_char.substring(1), 8));//переводим код в символьный вид
    }
    catch(NumberFormatException e){}    
    }
    else c = s_char.charAt(0);//присвоили переменной первый символ этой строки
    
    KeyCode keycode = KeyCode.find_keycode(s_keycode.toUpperCase());
    KeyShiftState shiftstate = KeyShiftState.find_keyshiftstate(s_shiftstate.toUpperCase());
    Finger finger = Finger.find_finger(s_finger.toUpperCase());
    GlyphGroup glyphgroup = GlyphGroup.find_glyphgroup(s_glyphgroup.toUpperCase());
    ...
    glyphs_by_char.put(glyph.get_character(), glyph);//поиск значения по символу
    Hashtable hashtable = get_table(glyph.get_shiftstate());//создаём промежуточную таблицу
    hashtable.get(glyph.get_keycode());
    }
